...
const MarksManagement = () => {
  ...
  const examTypes = ["Mid-term", "Final", "Unit Test 1", "Unit Test 2", "Assignment"];
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-100">
          ...
          <p className="text-2xl font-bold text-green-600">
            {Math.round(filteredStudents.reduce((sum, s) => sum + s.marks, 0) / filteredStudents.length || 0)}%
          </p>
        </div>
        <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-100">
          <div className="flex items-center justify-between mb-2">
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-slate-50">
              <tr>
                <th className="text-left py-4 px-6 text-sm font-semibold text-slate-600">Roll No</th>
                <th className="text-left py-4 px-6 text-sm font-semibold text-slate-600">Student ID</th>
                <th className="text-left py-4 px-6 text-sm font-semibold text-slate-600">Name</th>
                <th className="text-center py-4 px-6 text-sm font-semibold text-slate-600">Marks %</th>
            ...
            <tbody>
              {filteredStudents.map((student) => (
                <tr key={student.id} className="border-b border-slate-100 hover:bg-slate-50">
                  <td className="py-4 px-6 text-sm font-medium">{student.rollNo}</td>
                  <td className="py-4 px-6 text-sm text-slate-600">{student.id}</td>
                  <td className="py-4 px-6 text-sm font-medium">{student.name}</td>
                  ...
                  <td className="py-4 px-6 text-center text-sm font-medium">{student.cgpa}</td>
                  <td className="py-4 px-6 text-center">
                    <span
                  ...
                  <td className="py-4 px-6 text-center">
                    {editingId === student.id ? (
                      ...
                      <button
                        onClick={() => handleEditMarks(student.id, student.marks)}
